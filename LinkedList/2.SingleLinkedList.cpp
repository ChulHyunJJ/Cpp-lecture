...
/*
 숙제 : 학생관리 프로그램을 더블 링크드리스트로 만들어보기
 */
...
/*
 ...
 각 노드의 특징은 다음 노드를 알 수 있게 된다. (다음 노드의 메모리 주소를 저장한다는 의미)
 리스트는 선형구조로 되어있다. 그렇기 때문에 배열처럼 특정 요소에 바로 접근이 불가능하다.
 무조건 앞에서 차례대로 타고 들어가야 한다.
...
{
    // 포인터는 가급적이면 초기화할 때 NULL(0) 으로 초기화 해두고 쓰는 것이 좋다.
    // 왜냐하면 0은 false 0이 아닌 모든 수는 true 이기 때문이다.
    // 초기화를 하지 않으면 쓰레기 값이 들어가있는데 이 쓰레기 값도 true 이다.
    pList->pBegin = NULL;
    pList->pEnd = NULL;
    pList->iSize = 0;
}
...
{
    // 앞선 enter를 입력으로 받아들이는 경우가 있어 삭제해준다.
    cin.clear();
    cin.ignore(1024,'\n');
...
void Insert(PLIST pList)
{
    ...
    // 그래서 다음 노드는 NULL 로 초기화하고 정보는 위에서 입력받은 학생 정보를 준다.
    pNode->pNext = NULL;
    ...
    while (pNode != NULL)
    {
        if (strcmp(pNode->tStudent.strName, strName) == 0)
        {
            ...
            if (pPrev == NULL)
            {
                ...
                // 이전 노드가 있을 경우 이전 노드의 다음을 지운 노드의 다음으로 연결해준다.
                else
                {
                    delete pNode;
                    pPrev->pNext = pNext;
